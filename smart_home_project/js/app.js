...
function isRoomOffline(roomData) {
  return (
    !roomData || roomData.status === "offline" || roomData.temperature == null
  );
}
...
function renderSensorAlert() {
  ...
  if (isOffline && rt && rt.lastSeen) {
    const ls = new Date(rt.lastSeen).toLocaleTimeString("id-ID", {
      hour: "2-digit",
      minute: "2-digit",
      second: "2-digit",
    });
    sensorAlert.textContent = `⚠ Ruang Tamu — Sensor DHT11 Offline · last seen ${ls} · cek kabel D4 / WiFi & MQTT (10.89.145.239:1883)`;
  } else if (isOffline) {
    sensorAlert.textContent =
      "⚠ Ruang Tamu — Sensor DHT11 Offline · cek kabel D4 / koneksi WiFi & MQTT (10.89.145.239:1883)";
  }
}
...
function renderSummary() {
  ...
  const onlineTemps = Object.keys(rooms)
    .filter((r) => !isRoomOffline(rooms[r]))
    .map((r) => rooms[r].temperature);
  const onlineHumids = Object.keys(rooms)
    .filter((r) => !isRoomOffline(rooms[r]))
    .map((r) => rooms[r].humidity);

  if (onlineTemps.length > 0) {
    const avgTemp = onlineTemps.reduce((a, b) => a + b, 0) / onlineTemps.length;
    const avgHumid =
      onlineHumids.reduce((a, b) => a + b, 0) / onlineHumids.length;
    ...
    const sub = document.querySelector(
      ".summary-grid .summary-card:nth-child(1) .ssub",
    );
    if (sub) sub.textContent = onlineTemps.length + "/5 ruangan online";
    document.querySelector(
      ".summary-grid .summary-card:nth-child(2) .ssub",
    ).textContent = onlineTemps.length + "/5 ruangan online";
  } else {
  ...
  document.getElementById("totalWatt").textContent =
    totalWatt.toLocaleString("id-ID") + " W";
  ...
  Object.keys(rooms).forEach((r) => {
    ...
    if (rooms[r].temperature > hottestTemp) {
      hottestTemp = rooms[r].temperature;
      hottest = r;
    }
  });
  document.getElementById("hottestRoom").textContent = hottest
    ? ROOM_LABELS[hottest] || "—"
    : "—";
  document.getElementById("hottestTemp").textContent = hottest
    ? hottestTemp.toFixed(1) + "°C"
    : "—";

  const motion =
    latestData.pintu_masuk && latestData.pintu_masuk.motion_detected;
  document.getElementById("doorSummary").textContent = motion
    ? "Terdeteksi"
    : "Clear";
  document
    .getElementById("doorSummaryCard")
    .classList.toggle("alert", !!motion);
}
...
function renderFloorplan() {
  ...
  const motion =
    latestData.pintu_masuk && latestData.pintu_masuk.motion_detected;
  document.getElementById("doorText").textContent = motion
    ? "Gerakan terdeteksi"
    : "Tidak ada gerakan";
...
function renderDetail() {
  ...
  if (!selectedRoom || !latestData) {
    container.innerHTML =
      '<div class="detail-empty">Pilih ruangan di denah<br>untuk melihat detail</div>';
    return;
  }
  ...
  if (offline && selectedRoom === "ruang_tamu") {
    const lastSeenTxt = roomData.lastSeen
      ? formatWaktu(roomData.lastSeen)
      : "—";
    container.innerHTML = `
...
async function fetchData() {
  try {
    ...
    const label = t.toLocaleTimeString("id-ID", {
      hour: "2-digit",
      minute: "2-digit",
      second: "2-digit",
    });
...
document
  .getElementById("exportDoorBtn")
  .addEventListener("click", () => exportDoorDetailCSV(historyData));
document
  .getElementById("exportDoorHourlyBtn")
  .addEventListener("click", () => exportDoorHourlyCSV(historyData));
...
async function fetchHistory() {
  try {
    ...
    document
      .querySelectorAll("[data-export-room]")
      .forEach((btn) => (btn.disabled = validRoomEntries === 0));
